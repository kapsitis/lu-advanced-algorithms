...
\begin{center}
{\Large Handout, Week07: Kosaraju Algorithm}\\
%{\Large Published on 2021-11-01,}\\
...
\vspace{10pt}
{\bf (A)} Run the DFS traversal algorithm on the graph $G$. 
Mark each vertex 
...
\vspace{10pt}
{\bf (B)} Draw the transposed directed graph (same vertices, but each arrow points
...
(the finishing time for the DFS algorithm in step {\bf (A)}). 
...
\vspace{10pt}
{\bf (C)} List all the strongly connected components (they are 
